        ...
        )
{
    ...
    if (options.bm_cpu) {
        ...
        std::cout << "  residue: " << fabs( (sum_cl - sum_cpp) / sum_cpp )
                  << std::endl << std::endl;
    }
...
void run_tests(const vex::Context &ctx, vex::profiler<> &prof, int n_size)
{
    ...
    if (options.bm_sort) {
        ...
        benchmark_sort<real>(ctx, prof, n_size);
        prof.toc("Sorting");
    }
    ...
    if (options.bm_scan) {
        ...
        benchmark_scan<real>(ctx, prof, n_size);
        prof.toc("Scanning");
    }
...
int main(int argc, char *argv[]) {
    ...
    desc.add_options()
        ...
        ("bm_saxpy",
            po::value<bool>(&options.bm_saxpy)->default_value(false),
            "benchmark SAXPY (on/off)"
            )
        ("bm_vec",
            po::value<bool>(&options.bm_vector)->default_value(false),
            "benchmark vector arithmetics (on/off)"
            )
        ("bm_rdc",
            po::value<bool>(&options.bm_reductor)->default_value(false),
            "benchmark reduction (on/off)"
            )
        ("bm_stn",
            po::value<bool>(&options.bm_stencil)->default_value(false),
            "benchmark stencil convolution (on/off)"
            )
        ("bm_spm",
            po::value<bool>(&options.bm_spmv)->default_value(false),
            "benchmark sparse matrix - vector product (on/off)"
            )
        ("bm_rng",
            po::value<bool>(&options.bm_rng)->default_value(false),
